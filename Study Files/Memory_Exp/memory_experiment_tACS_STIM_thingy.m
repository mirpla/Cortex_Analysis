...
try
    ...
    for block = 1+((length(dir('gtb_s1_block*'))/2)*(session_num-1)):(length(dir('gtb_s1_block*'))/2)+((length(dir('gtb_s1_block*'))/2)*(session_num-1))
        %load file
        cd([Basepath 'Memory_Exp\csv/subj',num2str(participant_num)]);
...
%             %------------------------------------------------------------------------------------------------------
%             % read audio file, call it wav_orig, sample rate is Fs
%             [wav_orig,Fs]=audioread([Basepath 'Materials/sounds\' syncfile.soundfolder{shuffler(trial),1} '/' syncfile.soundname{shuffler(trial),1}]); 
%             % prepare to play the sound by adding it to the audio buffer in advance
%             PsychPortAudio('FillBuffer', pahandle, wav_orig');
% 
%             %------------------------------------------------------------------------------------------------------
